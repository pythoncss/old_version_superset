...
export const controls = {
  ...
  columns_and_metrics_x: {
    ...
    mapStateToProps: (state) => {
      let choices = [];
      //console.log('str',state.datasource); //To view output in console
      //console.log('str',state.datasource.metrics);
      //console.log('str',state.datasource.columns);
      //console.log('str',state.datasource.type);
      if (state.controls && state.datasource) {
        const gbSet = new Set((state.controls.groupby || {}).value || []);
        const gbSelectables = columnChoices(state.datasource).filter(elem => gbSet.has(elem[0])); //edited_@ishi

        const mSet = new Set((state.controls.metrics || {}).value || []);
        const mSelectables = state.datasource.metrics_combo.filter(elem => mSet.has(elem[0])); //@ishi

        const cSet = new Set((state.controls.all_columns || {}).value || []);
        const cSelectables = columnChoices(state.datasource).filter(elem => cSet.has(elem[0])); //changed__@ishi
 ...
 type: 'SelectControl',
    ...
    mapStateToProps: (state) => {
      ...
      if (state.controls && state.datasource) {
        const gbSet = new Set((state.controls.groupby || {}).value || []);
        const gbSelectables = columnChoices(state.datasource).filter(elem => gbSet.has(elem[0]));

        const mSet = new Set((state.controls.metrics || {}).value || []);
        const mSelectables = state.datasource.metrics_combo.filter(elem => mSet.has(elem[0]));

        const cSet = new Set((state.controls.all_columns || {}).value || []);
        const cSelectables = columnChoices(state.datasource).filter(elem => cSet.has(elem[0]));
        choices = [...gbSelectables, ...mSelectables, ...cSelectables];
      } else {
